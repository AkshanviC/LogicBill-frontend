...
const fetchInvoices = (page: number, limit: number) =>
  apiFetch<PaginatedResponse>(`${API_BASE}?page=${page}&limit=${limit}`);

const fetchInvoiceById = (id: number) =>
  apiFetch<Invoice>(`${API_BASE}/${id}`);
...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .il-table th.sortable { cursor: pointer; }
  .il-table th.sortable:hover { color: var(--accent); }
  .il-table tbody tr {
    border-bottom: 1px solid var(--border);
  ...
  .il-page-btns { display: flex; gap: 6px; }
  .il-page-btn {
    min-width: 32px;
    height: 32px;
    padding: 0 8px;
  ...
  .il-page-btn:hover:not(:disabled) {
    border-color: var(--accent);
    color: var(--accent);
  }
  .il-page-btn.active {
...
const generateMockInvoices = (): Invoice[] =>
  Array.from({ length: 23 }, (_, i) => ({
    id: i + 1,
    templateDetails: { layout: "standard", color: "#333" },
    createdBy: Math.floor(Math.random() * 5) + 1,
    templateId: Math.floor(Math.random() * 3) + 1,
    fromAddress: `${100 + i} MG Road, Chennai`,
    toAddress: `${200 + i} Anna Nagar, Chennai`,
    trailerNo: `TRL-${1000 + i}`,
    lrNo: `LR-${2000 + i}`,
    invoiceNo: `INV-${3000 + i}`,
    docNo: `DOC-${4000 + i}`,
    shipmentNo: `SHP-${5000 + i}`,
    weigth: Math.floor(Math.random() * 5000) + 100,
    trailers: Math.floor(Math.random() * 10) + 1,
    rate: Math.floor(Math.random() * 500) + 50,
    amount: Math.floor(Math.random() * 100000) + 5000,
    prorate: Math.floor(Math.random() * 1000),
    loadingCharge: Math.floor(Math.random() * 2000),
    cgst: 9,
    sgst: 9,
    createdAt: new Date(Date.now() - Math.random() * 1e10).toISOString(),
  }));

const MOCK_DATA = generateMockInvoices();

function mockFetchInvoices(page: number, limit: number, search: string): PaginatedResponse {
  const filtered = search
    ? MOCK_DATA.filter(
        (inv) =>
          inv.invoiceNo?.toLowerCase().includes(search.toLowerCase()) ||
          inv.lrNo?.toLowerCase().includes(search.toLowerCase()) ||
          inv.shipmentNo?.toLowerCase().includes(search.toLowerCase()) ||
          inv.fromAddress?.toLowerCase().includes(search.toLowerCase()) ||
          inv.toAddress?.toLowerCase().includes(search.toLowerCase()) ||
...
interface DetailRowProps { label: string; value?: string | number | null; accent?: boolean; full?: boolean; }

const DetailRow: React.FC<DetailRowProps> = ({ label, value, accent, full }) => (
  <div className={`il-detail-item${full ? " full" : ""}`}>
    <div className="il-detail-label">{label}</div>
    <div className={`il-detail-value${accent ? " accent" : ""}${!value && value !== 0 ? " empty" : ""}`}>
      {value !== undefined && value !== null && value !== "" ? String(value) : "—"}
    </div>
  </div>
);

interface ViewModalProps { invoice: Invoice; onClose: () => void; onEdit: () => void; }

const ViewModal: React.FC<ViewModalProps> = ({ invoice, onClose, onEdit }) => (
  <div className="il-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
    <div className="il-modal">
      <div className="il-modal-header">
        <div className="il-modal-title">
          Invoice Details
          <span className="il-modal-tag">#{invoice.id}</span>
        </div>
        <button className="il-modal-close" onClick={onClose}><IconX /></button>
      </div>
      <div className="il-modal-body">
        <div className="il-detail-grid">
          <DetailRow label="Invoice No" value={invoice.invoiceNo} accent />
          <DetailRow label="Doc No" value={invoice.docNo} />
          <DetailRow label="LR No" value={invoice.lrNo} />
          <DetailRow label="Shipment No" value={invoice.shipmentNo} />
          <DetailRow label="Trailer No" value={invoice.trailerNo} />
          <DetailRow label="From Address" value={invoice.fromAddress} />
          <DetailRow label="To Address" value={invoice.toAddress} />
          <DetailRow label="Weight (kg)" value={invoice.weigth} />
          <DetailRow label="Trailers" value={invoice.trailers} />
          <DetailRow label="Rate" value={invoice.rate} />
          <DetailRow label="Amount" value={invoice.amount} accent />
          <DetailRow label="Pro-rate" value={invoice.prorate} />
          <DetailRow label="Loading Charge" value={invoice.loadingCharge} />
          <DetailRow label="CGST (%)" value={invoice.cgst} />
          <DetailRow label="SGST (%)" value={invoice.sgst} />
          <DetailRow label="Others" value={invoice.others} full />
          <DetailRow label="Template ID" value={invoice.templateId} />
          <DetailRow label="Created By" value={invoice.createdBy} />
          {invoice.createdAt && (
            <DetailRow label="Created At" value={new Date(invoice.createdAt).toLocaleString()} full />
          )}
        </div>
      </div>
...
interface EditModalProps { invoice: Invoice; onClose: () => void; onSave: (updated: Invoice) => void; }

const EditModal: React.FC<EditModalProps> = ({ invoice, onClose, onSave }) => {
  const [form, setForm] = useState<Invoice>({ ...invoice });
  const [saving, setSaving] = useState(false);

  const set = (key: keyof Invoice) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const val = e.target.value;
    setForm((prev) => ({
      ...prev,
      [key]: ["weigth","trailers","rate","amount","prorate","loadingCharge","cgst","sgst","createdBy","templateId"].includes(key)
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      // Replace with real API: await updateInvoice(invoice.id, form);
      ...
      <div className="il-modal">
        ...
        <div className="il-modal-body">
          <div className="il-form-grid">
            ...
            {field("Trailer No", "trailerNo")}
            {field("From Address", "fromAddress", "text", true)}
            {field("To Address", "toAddress", "text", true)}
            {field("Weight (kg)", "weigth", "number")}
            {field("Trailers", "trailers", "number")}
            {field("Rate", "rate", "number")}
            {field("Amount", "amount", "number")}
            {field("Pro-rate", "prorate", "number")}
            {field("Loading Charge", "loadingCharge", "number")}
            {field("CGST (%)", "cgst", "number")}
            {field("SGST (%)", "sgst", "number")}
            <div className="il-form-group full">
              <label className="il-form-label">Others</label>
              <textarea
                className="il-form-input il-form-textarea"
                value={form.others ?? ""}
                onChange={set("others")}
...
const InvoiceList: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setError(null);
    try {
      // Using mock data — replace with: fetchInvoices(page, LIMIT).then(...)
      const res = mockFetchInvoices(page, LIMIT, debouncedSearch);
  ...
  const renderPageButtons = () => {
    ...
    return pages.map((p, i) =>
      p === "…" ? (
        <span key={`ellipsis-${i}`} style={{ color: "var(--text-muted)", padding: "0 4px", fontFamily: "var(--font-mono)", fontSize: 12 }}>…</span>
      ) : (
                ...
                {loading ? (
                  ...
                  invoices.map((inv) => (
                    <tr key={inv.id}>
                      ...
                      <td>{inv.lrNo ?? "—"}</td>
                      <td>{inv.shipmentNo ?? "—"}</td>
                      <td style={{ maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis" }}>{inv.fromAddress ?? "—"}</td>
                      <td style={{ maxWidth: 140, overflow: "hidden", textOverflow: "ellipsis" }}>{inv.toAddress ?? "—"}</td>
                      <td><span className="il-amount">{fmt(inv.amount)}</span></td>
                      <td>{inv.cgst !== undefined ? `${inv.cgst}%` : "—"}</td>
                      <td>{inv.sgst !== undefined ? `${inv.sgst}%` : "—"}</td>
                      <td>
          ...
          {!loading && invoices.length > 0 && (
            <div className="il-pagination">
              ...
              <div className="il-page-btns">
                <button className="il-page-btn" onClick={() => setPage((p) => p - 1)} disabled={page === 1}>
                  <IconChevronLeft />
                </button>
                {renderPageButtons()}
                <button className="il-page-btn" onClick={() => setPage((p) => p + 1)} disabled={page === totalPages}>
                  <IconChevronRight />
                </button>
              </div>
            </div>
